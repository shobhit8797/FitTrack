...
> Gemini/OpenRouter now power **food estimation only** (meal photo, label, text).

### Configure the Lyzr plan agents (workout + diet)

Workout and diet plans are generated by two separate **Lyzr Studio** agents
(the "Fitness Architect" and "Nutrition Architect"). The app calls a Cloud
Function, which asks the agent for a strict-JSON plan and stores it per user.

```bash
# Agent IDs are config (functions/.env):
#   LYZR_WORKOUT_AGENT_ID = <workout agent id>
#   LYZR_DIET_AGENT_ID     = <diet agent id>
#   LYZR_BASE_URL          = https://agent-prod.studio.lyzr.ai/v3/inference/chat/
# The API key is a secret:
firebase functions:secrets:set LYZR_API_KEY
```

For most reliable one-shot JSON, set each agent's response format to JSON in
Lyzr Studio; the request also forces JSON in the message and the backend parses
tolerantly, so markdown-configured agents still work.
...
- ✅ iOS: models, services (Auth/Repository/Functions/HealthKit), design system,
  sign-in, onboarding, Today dashboard (with in-place meal + workout logging),
  workout plan + session logging, **diet plan tab**, progress (weight chart),
  settings (incl. on-demand **Generate workout / diet plan**).
- ✅ Plans: generated on demand from Settings (workout, diet, or both) by the two
  Lyzr agents; no auto-generation at onboarding.
